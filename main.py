...
async def main():
    if os.path.exists(SESSION_FILE):
        ...
        print("Session not found. Please login via the helper bot using /login")
        # فقط وب‌سرور را اجرا کن تا سرویس در Render زنده بمونه و پورت باز باشه
        await start_web_server()
        # برای همیشه منتظر بمان (تا زمانی که سرویس ری‌استارت شود یا کاربر لاگین کند)
        await asyncio.Event().wait()   # یا می‌توانید از while True با sleep استفاده کنید
