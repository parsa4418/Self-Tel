...
from lib import *
from lib.Information import client as userbot_client
...
# ================= Userbot Login Flow =================
# اگر فایل session یوزربات وجود نداشته باشه، این بخش فعال میشه
# و با ارسال /login به همین بات، مراحل شماره/کد/رمز دومرحله‌ای رو می‌پرسه.

SESSION_FILE = 'TRself-MT.session'
login_state = {'step': None, 'phone': None, 'phone_code_hash': None}
...
async def start_login(event):
    ...
    if os.path.exists(SESSION_FILE):
        await event.respond('✅ قبلاً لاگین شدی، نیازی به این کار نیست.')
        return
    await userbot_client.connect()
    login_state['step'] = 'phone'
    await event.respond('شماره اکانتت رو با کد کشور بفرست (مثلا +989123456789)')

@client.on(events.NewMessage)
async def handle_login_steps(event):
    if event.sender_id != admin_user_id or login_state['step'] is None:
        return
    ...
    if login_state['step'] == 'phone':
        try:
            sent = await userbot_client.send_code_request(text)
            login_state['phone'] = text
            login_state['phone_code_hash'] = sent.phone_code_hash
            login_state['step'] = 'code'
            await event.respond('کدی که تلگرام برات فرستاد رو بفرست')
    ...
    if login_state['step'] == 'code':
        try:
            ...
            login_state['step'] = None
            await event.respond('✅ لاگین با موفقیت انجام شد! سرویس رو یه بار ریستارت کن تا کامل بالا بیاد.')
        except SessionPasswordNeededError:
            login_state['step'] = 'password'
            await event.respond('اکانتت رمز دومرحله‌ای (2FA) داره، رمزشو بفرست')
        except Exception as e:
            await event.respond(f'❈ خطا: {e}\nدوباره /login رو بزن.')
            login_state['step'] = None
        return
    ...
    if login_state['step'] == 'password':
        try:
            await userbot_client.sign_in(password=text)
            login_state['step'] = None
            await event.respond('✅ لاگین با موفقیت انجام شد! سرویس رو یه بار ریستارت کن تا کامل بالا بیاد.')
        except Exception as e:
            await event.respond(f'❈ خطا: {e}\nدوباره /login رو بزن.')
