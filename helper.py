from telethon import Button, types, events, TelegramClient
from telethon.errors import SessionPasswordNeededError
from lib.Information import api_id, api_hash, admin_user_id, bot_token
...
# ================= کلاینت هلپر (برای لاگین) =================
helper_client = TelegramClient('helper_session', api_id, api_hash)
...
async def handle_login_steps(event):
    ...
    if login_state['step'] == 'phone':
        try:
            sent = await helper_client.send_code_request(text)
    ...
    if login_state['step'] == 'code':
        try:
            await helper_client.sign_in(
                phone=login_state['phone'],
                code=text,
                phone_code_hash=login_state['phone_code_hash']
            )
            # ✅ لاگین موفق – کپی کردن فایل سشن
            shutil.copy2('helper_session.session', SESSION_FILE)
            login_state['step'] = None
            await event.respond('✅ لاگین با موفقیت انجام شد! سرویس رو یک بار ری‌استارت کن تا کامل بالا بیاد.')
        except SessionPasswordNeededError:
            login_state['step'] = 'password'
            await event.respond('اکانتت رمز دو مرحله‌ای داره، رمزشو بفرست')
        except Exception as e:
            await event.respond(f'❈ خطا: {e}\nدوباره /login رو بزن.')
            login_state['step'] = None
        return
    ...
    if login_state['step'] == 'password':
        try:
            await helper_client.sign_in(password=text)
            shutil.copy2('helper_session.session', SESSION_FILE)
            login_state['step'] = None
            await event.respond('✅ لاگین با موفقیت انجام شد! سرویس رو یک بار ری‌استارت کن تا کامل بالا بیاد.')
        except Exception as e:
...
# ============== بقیه‌ی کد (InlineQuery, CallbackQuery, ...) ==============
# ... (همان کدهای قبلی که از `client` استفاده می‌کنند) ...
